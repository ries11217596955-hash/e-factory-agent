...
def process_job(job_dir: Path, restart: bool = False) -> int:
    ...
    append_log(
        log_path,
        f"JOB_START job_id={state.get('job_id')} stage={state.get('stage')} "
        f"status={state.get('status')} restart={restart}",
    )

    if state["status"] in {"fail", "hold"} and not restart:
    ...
    while state.get("stage") is not None:
        ...
        try:
            ...
            append_log(
                log_path,
                f"STAGE_RESULT stage={current_stage} status={result_status} message={result_message}",
            )
            ...
            if current_stage == "audit":
                ...
                if state["verdict"] == "HOLD":
                    state["status"] = "hold"
                    state["error"] = "Audit verdict = HOLD"
                    save_state(job_dir, state)
...
def main() -> int:
    ...
    parser.add_argument(
        "--restart",
        action="store_true",
        help="Reset outputs/reports/logs and rerun the job from brief.",
    )
