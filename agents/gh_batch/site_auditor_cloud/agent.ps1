...
function Get-RepoRoot {
    ...
    for ($i = 0; $i -lt 8; $i++) {
        ...
        $eleventy = Join-Path $current ".eleventy.js"
        $git = Join-Path $current ".git"

        if ((Test-Path $src) -and ((Test-Path $pkg) -or (Test-Path $eleventy) -or (Test-Path $git))) {
            return $current
        }
...
function Get-RepoSlug {
    if (-not [string]::IsNullOrWhiteSpace($env:TARGET_REPO_SLUG)) { return [string]$env:TARGET_REPO_SLUG }
    $root = Get-ScriptRoot
    $fixed = Join-Path $root "repos.fixed.json"
    if (Test-Path $fixed) {
        try {
            $items = Get-Content -Path $fixed -Raw -Encoding UTF8 | ConvertFrom-Json
            foreach ($x in (Normalize $items)) {
                if ([string]$x -match '/automation-kb$') {
                    return [string]$x
                }
            }
            foreach ($x in (Normalize $items)) {
                if (-not [string]::IsNullOrWhiteSpace([string]$x)) { return [string]$x }
            }
        }
        catch {}
    }
...
function Test-IsLikelyTargetRepoRoot {
    param([string]$Path)
    ...
    $gh = Join-Path $Path ".github"
    $agentMarker = Join-Path $Path "agents/gh_batch/site_auditor_cloud"

    if (Test-Path $agentMarker) { return $false }
    if (-not (Test-Path $src)) { return $false }
    if ((Test-Path $pkg) -or (Test-Path $eleventy) -or (Test-Path $gh)) { return $true }
    return $false
}
...
function Resolve-TargetRepoRoot {
    ...
    foreach ($value in @(
        ...
        $env:SITE_REPO_PATH,
        (Join-Path (Get-ScriptRoot) "target_repo"),
        (Join-Path $PWD "target_repo"),
        (Join-Path $env:GITHUB_WORKSPACE "target_repo"),
        (Join-Path $env:GITHUB_WORKSPACE "automation-kb"),
        (Join-Path $env:GITHUB_WORKSPACE "site_repo")
    )) {
    ...
    foreach ($c in $unique) {
        if (Test-IsLikelyTargetRepoRoot -Path $c) {
            return [pscustomobject]@{
                repo_root = $c
                repo_source = 'WORKFLOW_CHECKOUT'
                repo_slug = $slug
    ...
    foreach ($c in $unique) {
        $resolved = Get-RepoRoot -StartPath $c
        if (Test-IsLikelyTargetRepoRoot -Path $resolved) {
            return [pscustomobject]@{
                repo_root = $resolved
                repo_source = 'PATH_DISCOVERY'
                repo_slug = $slug
                target_repo_bound = $true
                binding_error = ''
            }
    ...
    return [pscustomobject]@{
        ...
        binding_error = 'TARGET_REPO_NOT_BOUND'
    }
}
...
function Get-VisibleText {
    ...
    foreach ($name in @('visibleText','bodyText','text','innerText')) {
...
function Analyze-RepoHygiene {
    ...
    if ($null -eq $Binding -or -not $Binding.target_repo_bound -or [string]::IsNullOrWhiteSpace([string]$Binding.repo_root)) {
        return [pscustomobject]@{
            repo_root = ''
            repo_source = ''
            repo_slug = $(if ($null -ne $Binding) { [string]$Binding.repo_slug } else { '' })
            target_repo_bound = $false
            binding_error = $(if ($null -ne $Binding) { [string]$Binding.binding_error } else { 'TARGET_REPO_NOT_BOUND' })
...
function Test-UiContamination {
    ...
    $haystack = @($Title, $Path, $VisibleText) -join "`n"
    $hits = @()
    foreach ($m in $markers) {
        if (-not [string]::IsNullOrWhiteSpace($haystack) -and $haystack -like "*$m*") {
            $hits += $m
        }
    }
...
function Get-Findings {
    ...
    foreach ($i in (Normalize $items)) {
        ...
        $path = Get-PathFromItem $i
        $title = [string]$i.title
        $visibleText = Get-VisibleText $i

        $visual = "ok"
        if ($path -eq "/") {
            if ($len -lt 180 -and $links -lt 2) { $visual = "empty" }
            elseif ($len -lt 500 -and $links -lt 6) { $visual = "thin" }
            elseif ($img -eq 0 -or $len -lt 900) { $visual = "weak" }
        }
        elseif ($path -eq "/search/") {
            if ($len -lt 120 -and $links -lt 1) { $visual = "empty" }
            elseif ($len -lt 260) { $visual = "thin" }
            elseif ($img -eq 0 -and $links -lt 4) { $visual = "weak" }
        }
        else {
            if ($img -eq 0 -and $len -lt 180) { $visual = "empty" }
            elseif ($len -lt 350) { $visual = "thin" }
            elseif ($img -eq 0) { $visual = "weak" }
        }

        $pageType = Get-PageType -Path $path -Len $len -Links $links
        $uiContaminationHits = Test-UiContamination -Title $title -Path $path -VisibleText $visibleText
        ...
        if ($path -eq "/") {
            $hasProblemFrame = ($len -ge 300)
            $hasSolutionDepth = ($len -ge 650)
            $hasNextStep = ($links -ge 3)
            $hasValueStatement = ($len -ge 400)
            $hasCTA = ($links -ge 3)
        }
        elseif ($path -eq "/search/") {
            $hasProblemFrame = ($len -ge 180)
            $hasSolutionDepth = ($len -ge 260)
            $hasNextStep = ($links -ge 1)
            ...
            if ($links -ge 4) { $hasNextStep = $true }
            if ($len -ge 500) { $hasValueStatement = $true }
            if ($links -ge 6 -or $path -match '/start-here/|/pricing/|/contact/|/demo/|/consult|/signup|/subscribe') { $hasCTA = $true }
        }
        ...
        $fakeShell = $false
        if (($pageType -eq "SCAFFOLD" -or $pageType -eq "EMPTY") -and $links -gt 0 -and $len -lt 220) {
            $fakeShell = $true
        }
...
function Get-Scores {
    ...
    foreach ($i in (Normalize $items)) {
        ...
        $band = "ok"
        if ($p -eq "/") {
            if ($len -lt 220 -and $links -lt 2) { $band = "bad" }
            elseif ($len -lt 500 -or $links -lt 3) { $band = "thin" }
        }
        elseif ($p -eq "/search/") {
            if ($len -lt 120) { $band = "bad" }
            elseif ($len -lt 260) { $band = "thin" }
        }
        else {
            if ($len -lt 220) { $band = "bad" }
            elseif ($len -lt 500) { $band = "thin" }
        }
...
function Invoke-SiteAuditor {
    ...
    $binding = Resolve-TargetRepoRoot
    $repoAudit = Analyze-RepoHygiene -Binding $binding
    ...
    Write-Host ("REPO_ROOT=" + $repoAudit.repo_root)
    Write-Host ("REPO_BOUND=" + [string]$repoAudit.target_repo_bound)
    if (-not $repoAudit.target_repo_bound) {
        Write-Host ("BINDING_ERROR=" + $repoAudit.binding_error)
    }
    Write-Host "DONE"
}
