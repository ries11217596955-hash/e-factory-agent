...
function Find-GovernanceTopLevelItems {
    param([string]$RepoRoot)

    $hits = @()
    $known = @('00__AI__MEMORY', '00__AI__CONTROL_LOOP.md', 'STRATEGY_BOARD.md')
    foreach ($name in $known) {
        if (Test-Path -LiteralPath (Join-Path $RepoRoot $name)) { $hits += $name }
    }
    return @($hits | Select-Object -Unique)
}
...
function Find-SuspiciousTopLevelItems {
    ...
    $allow = @(
        'README.md','package.json','package-lock.json','.gitignore','.github','src','assets',
        'scripts','nav.js','index.11ty.js','CONTRIBUTING.md','tools','test','.eleventy.js',
        '.eleventyignore','.eslintignore','.eslintrc.cjs','.prettierignore','.prettierrc',
        '00__AI__MEMORY','00__AI__CONTROL_LOOP.md','STRATEGY_BOARD.md'
    )
    ...
    foreach ($item in $topItems) {
        ...
        if ($item.PSIsContainer) {
            $childSrc = Join-Path $item.FullName 'src'
            $childReadmeApply = Join-Path $item.FullName 'README_APPLY.txt'
            if ($name -match '^[A-Za-z0-9]{1,3}$') {
                $hits += $name
                continue
            }
            if ((Test-Path -LiteralPath $childSrc) -or (Test-Path -LiteralPath $childReadmeApply)) {
                $hits += $name
                continue
            }
        }
...
function Analyze-RepoHygiene {
    ...
    $governance = Find-GovernanceTopLevelItems -RepoRoot $RepoRoot

    return [pscustomobject]@{
        repo_root = $RepoRoot
        suspicious_top_level_items = @($topSuspicious)
        governance_top_level_items = @($governance)
...
function Analyze-System {
    ...
    foreach ($f in (Normalize $findings)) {
        if ($f.path -eq '/' -and $f.has_next_step -and ($f.has_value_statement -or $f.links -ge 4)) { $entry = $true }
...
function Analyze-UserReality {
    ...
    foreach ($f in (Normalize $findings)) {
        if ($f.path -eq '/') {
            if (($f.band -eq 'bad') -or (-not $f.has_next_step) -or (-not $f.has_cta)) {
                $homepageFail = $true
                $entryClarityRoutes += $f.path
            }
        }

        if ((-not $f.has_problem_frame) -and ($f.band -eq 'bad' -or $f.visual -eq 'empty')) {
...
function Decide {
    ...
    if (-not $repoAudit.repo_clean -or -not $repoAudit.architecture_clean) {
        $core = 'Repo contains stray or patch-like root artifacts that do not belong to the canonical product surface.'
    }
    ...
    if (-not $repoAudit.repo_clean) {
        $p0 += 'Repo cleanliness failed: stray or patch-like root artifacts are present in the product repo.'
    }
    if (-not $repoAudit.architecture_clean) {
        $p0 += 'Architecture boundary failed: runtime or delivery directories are mixed into the repo root.'
    }
    ...
    $do = @()
    if (-not $repoAudit.repo_clean -or -not $repoAudit.architecture_clean) {
        $do += 'Remove stray patch/staging artifacts from repo root; keep governance files separate from accidental clone/staging folders.'
    }
    if ($ux.ui_contamination_routes.Count -gt 0) {
