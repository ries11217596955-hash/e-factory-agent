...
$legacyBundleRoot = Join-Path $PSScriptRoot 'audit_bundle'
$packagedScreenshotsRoot = Join-Path $PSScriptRoot 'screenshots'
$bundleRoot = $reportsRoot
...
$runReportCanonicalPath = Join-Path $bundleRoot 'RUN_REPORT.json'
...
function Write-RunReportJson {
    param(
        ...
        [string]$ErrorClass,
        [string]$Message,
        [object]$VisualArtifacts = $null
    )

    Ensure-Directory -Path $reportsRoot
    $payload = [ordered]@{
        schema_version = 'run_report.v1'
        run_id = if ([string]::IsNullOrWhiteSpace($RunId)) { 'bundle-run' } else { $RunId }
        final_status = if ([string]::IsNullOrWhiteSpace($FinalStatus)) { 'FAIL' } else { $FinalStatus }
        failed_step = if ([string]::IsNullOrWhiteSpace($FailedStep)) { 'NONE' } else { $FailedStep }
        error_class = if ([string]::IsNullOrWhiteSpace($ErrorClass)) { 'NONE' } else { $ErrorClass }
        message = if ([string]::IsNullOrWhiteSpace($Message)) { 'RUN_REPORT generated by run_bundle pipeline.' } else { $Message }
    }
    if ($null -ne $VisualArtifacts) {
        $payload.visual_artifacts = $VisualArtifacts
    }

    $payload | ConvertTo-Json -Depth 6 | Out-File -FilePath $runReportCanonicalPath -Encoding utf8
    Add-ExecutionLog "RUN_REPORT_JSON_WRITTEN path=$runReportCanonicalPath status=$($payload.final_status)"
}

function Ensure-RunReportBeforeValidation {
    if (Test-Path -Path $runReportCanonicalPath -PathType Leaf) {
        return
    }
    ...
    $fallback = [ordered]@{
        ...
        final_status = 'FAIL'
        failed_step = 'RUN_REPORT_MISSING'
        error_class = 'OUTPUT_LAYER_DESYNC'
        message = 'RUN_REPORT was missing and auto-generated before validation.'
    }

    Ensure-Directory -Path $reportsRoot
    $fallback | ConvertTo-Json -Depth 4 | Out-File -FilePath $runReportCanonicalPath -Encoding utf8
    Add-ExecutionLog "RUN_REPORT_FALLBACK_CREATED path=$runReportCanonicalPath"
}

function Invoke-ValidationStage {
    Add-ExecutionLog 'STAGE 4 (VALIDATION) started.'
    Ensure-RunReportBeforeValidation
    if (-not (Test-Path -Path $runReportCanonicalPath -PathType Leaf)) {
        Add-ExecutionLog "VALIDATION_FAIL missing=$runReportCanonicalPath"
        throw "Validation failed: missing canonical RUN_REPORT at $runReportCanonicalPath"
    }

    Add-ExecutionLog "VALIDATION_OK source=$runReportCanonicalPath"
    Add-ExecutionLog 'STAGE 4 (VALIDATION) completed.'
}
...
function Get-RepoScreenshotManifest {
    ...
    foreach ($sourceRoot in $sourceRoots) {
        ...
        $pngFiles = Get-ChildItem -Path $sourceRoot -Filter '*.png' -File -Recurse -ErrorAction SilentlyContinue | Sort-Object -Property FullName
        foreach ($pngFile in $pngFiles) {
            $relativeSegment = [System.IO.Path]::GetRelativePath($sourceRoot, $pngFile.FullName).Replace('\', '/')
            if ([string]::IsNullOrWhiteSpace($relativeSegment)) {
                $relativeSegment = $pngFile.Name
            }

            $canonicalPath = "screenshots/$relativeSegment"
            $canonicalKey = $canonicalPath.ToLowerInvariant()
            if ($canonicalKeys.ContainsKey($canonicalKey)) {
                continue
            }
            ...
            $manifest.Add([ordered]@{
                ...
                file_name = [System.IO.Path]::GetFileName($canonicalPath)
            })
        }
    }
    ...
    if ($manifest.Count -eq 0) {
        $fallbackNameCounts = @{}
        foreach ($sourceRoot in $fallbackSourceRoots) {
            if (-not (Test-Path -Path $sourceRoot -PathType Container)) {
                continue
            }
            ...
            foreach ($pngFile in $pngFiles) {
                ...
                if (-not $fallbackNameCounts.ContainsKey($baseName)) {
                    $fallbackNameCounts[$baseName] = 0
                }

                $fallbackNameCounts[$baseName] += 1
                $copyName = if ($fallbackNameCounts[$baseName] -eq 1) {
                    $baseName
                }
                else {
                    $stem = [System.IO.Path]::GetFileNameWithoutExtension($baseName)
                    $ext = [System.IO.Path]::GetExtension($baseName)
                    "$stem-$($fallbackNameCounts[$baseName])$ext"
                }

                $canonicalPath = "screenshots/$copyName"
                $canonicalKey = $canonicalPath.ToLowerInvariant()
                if ($canonicalKeys.ContainsKey($canonicalKey)) {
                    continue
                }
...
function Get-VisualArtifactContract {
    param(
        [object[]]$ScreenshotManifest
    )

    $repoReportsRoot = Join-Path (Join-Path $bundleRoot 'repo') 'reports'
    $auditResultPath = Join-Path $repoReportsRoot 'audit_result.json'
    $repoRunReportPath = Join-Path $repoReportsRoot 'RUN_REPORT.json'
    $auditResult = Get-JsonIfPresent -Path $auditResultPath
    $repoRunReport = Get-JsonIfPresent -Path $repoRunReportPath

    $visualActive = $false
    if ($null -ne $auditResult -and $null -ne $auditResult.PSObject.Properties['live']) {
        $liveLayer = $auditResult.live
        if ($null -ne $liveLayer -and $null -ne $liveLayer.PSObject.Properties['enabled']) {
            $visualActive = [bool]$liveLayer.enabled
        }
    }
    ...
    $addExpectedPath = {
        ...
        $clean = $PathValue.Trim().Replace('\', '/')
        if ($clean.StartsWith('reports/screenshots/')) {
            $clean = "screenshots/$($clean.Substring('reports/screenshots/'.Length))"
        }
        elseif ($clean.StartsWith('./reports/screenshots/')) {
            $clean = "screenshots/$($clean.Substring('./reports/screenshots/'.Length))"
        }
        elseif (-not $clean.StartsWith('screenshots/')) {
            return
        }

        $key = $clean.ToLowerInvariant()
        if (-not $seenExpected.ContainsKey($key)) {
            $seenExpected[$key] = $true
            $expectedPaths.Add($clean)
        }
    }
    ...
    if ($null -ne $auditResult -and $null -ne $auditResult.PSObject.Properties['live']) {
        $routeDetails = @()
        $liveLayer = $auditResult.live
        if ($null -ne $liveLayer -and $null -ne $liveLayer.PSObject.Properties['route_details']) {
            $routeDetails = @($liveLayer.route_details)
        }

        foreach ($route in $routeDetails) {
            if ($null -eq $route) { continue }
            if ($null -ne $route.PSObject.Properties['screenshots']) {
                foreach ($item in @($route.screenshots)) { & $addExpectedPath ([string]$item) }
            }
            if ($null -ne $route.PSObject.Properties['issue_screenshots']) {
                foreach ($item in @($route.issue_screenshots)) { & $addExpectedPath ([string]$item) }
            }
            if ($null -ne $route.PSObject.Properties['screenshot_map']) {
                foreach ($entry in $route.screenshot_map.PSObject.Properties) {
                    & $addExpectedPath ([string]$entry.Value)
                }
            }
        }
    }
    ...
    $packagedPaths = @()
    $seenPackaged = @{}
    foreach ($item in @($ScreenshotManifest)) {
        $relativePath = if ($null -ne $item -and $null -ne $item.PSObject.Properties['relative_path']) { [string]$item.relative_path } else { '' }
        if ([string]::IsNullOrWhiteSpace($relativePath)) { continue }
        $clean = $relativePath.Replace('\', '/')
        $key = $clean.ToLowerInvariant()
        if (-not $seenPackaged.ContainsKey($key)) {
            $seenPackaged[$key] = $true
            $packagedPaths += $clean
        }
    }
    ...
    foreach ($expected in @($expectedPaths)) {
        if (-not $seenPackaged.ContainsKey($expected.ToLowerInvariant())) {
            $missingPaths.Add($expected)
        }
    }

    if ($visualActive -and $expectedPaths.Count -eq 0 -and $packagedPaths.Count -eq 0) {
        $missingPaths.Add('screenshots/<missing-runtime-capture>')
    }

    $expectedCount = if ($visualActive) {
        [Math]::Max(1, $expectedPaths.Count)
    }
    else {
        $expectedPaths.Count
    }
    $packagedCount = $packagedPaths.Count
    $status = if ($visualActive -and ($packagedCount -eq 0 -or $missingPaths.Count -gt 0)) { 'FAIL' } else { 'PASS' }

    $contractNode = [ordered]@{
        visual_audit_active = [bool]$visualActive
        screenshots_expected = [int]$expectedCount
        screenshots_packaged = [int]$packagedCount
        screenshots_missing = [int]$missingPaths.Count
    ...
    if ($null -ne $auditResult) {
        $artifactsNode = [ordered]@{}
        if ($null -ne $auditResult.PSObject.Properties['artifacts'] -and $null -ne $auditResult.artifacts) {
            $rawArtifacts = $auditResult.artifacts
            if ($rawArtifacts -is [System.Collections.IDictionary]) {
                foreach ($key in $rawArtifacts.Keys) {
                    $artifactsNode[[string]$key] = $rawArtifacts[$key]
                }
            }
            else {
                foreach ($prop in $rawArtifacts.PSObject.Properties) {
                    $artifactsNode[[string]$prop.Name] = $prop.Value
                }
            }
        }
        $artifactsNode.screenshots_dir = 'screenshots'
        $artifactsNode.screenshots = @($packagedPaths)
        $artifactsNode.visual_contract = $contractNode
        if ($null -eq $auditResult.PSObject.Properties['artifacts']) {
            $auditResult | Add-Member -NotePropertyName artifacts -NotePropertyValue $artifactsNode
        }
        else {
            $auditResult.artifacts = $artifactsNode
        }
        $auditResult | ConvertTo-Json -Depth 14 | Out-File -FilePath $auditResultPath -Encoding utf8
    }

    if ($null -ne $repoRunReport) {
        if ($null -eq $repoRunReport.PSObject.Properties['visual_artifacts']) {
            $repoRunReport | Add-Member -NotePropertyName visual_artifacts -NotePropertyValue $contractNode
        }
        else {
            $repoRunReport.visual_artifacts = $contractNode
        }
        $repoRunReport | ConvertTo-Json -Depth 12 | Out-File -FilePath $repoRunReportPath -Encoding utf8
    }
...
function New-ReportLines {
    ...
    $lines.Add('')
    $lines.Add('EXECUTION LOG: reports/EXECUTION_LOG.txt')
    $lines.Add('MASTER SUMMARY: reports/master_summary.json')
    $lines.Add('BUNDLE STATUS: reports/audit_bundle_summary.json')
    $lines.Add('OPERATOR PRIORITY ACTIONS: reports/00_PRIORITY_ACTIONS.txt')
    $lines.Add('OPERATOR TOP ISSUES: reports/01_TOP_ISSUES.txt')
    $lines.Add('OPERATOR EXECUTIVE SUMMARY: reports/11A_EXECUTIVE_SUMMARY.txt')
    $lines.Add('ANALYST HANDOFF BRIEF: reports/12A_META_AUDIT_BRIEF.txt')
    $lines.Add('')
...
function Invoke-WritingStage {
    ...
    Ensure-Directory -Path $legacyBundleRoot
    if (Test-Path -Path $packagedScreenshotsRoot -PathType Container) {
        Remove-Item -Path (Join-Path $packagedScreenshotsRoot '*') -Recurse -Force -ErrorAction SilentlyContinue
    }
    ...
    if ($null -eq $Assembled.bundle_status.repo.visual_artifacts) {
        $Assembled.bundle_status.repo.visual_artifacts = [ordered]@{}
    }
    $Assembled.bundle_status.repo.visual_artifacts = $visualContract
    if ([string]$visualContract.status -eq 'FAIL') {
        $Assembled.bundle_status.repo.status = 'FAIL'
        $Assembled.bundle_status.repo.reason = "VISUAL_ARTIFACT_CONTRACT_FAIL expected=$($visualContract.screenshots_expected) packaged=$($visualContract.screenshots_packaged) missing=$($visualContract.screenshots_missing)"
        $Assembled.bundle_status.overall = 'FAIL'
        $Assembled.overall_status = 'FAIL'
        Add-ExecutionLog "VISUAL_ARTIFACT_CONTRACT_FAIL expected=$($visualContract.screenshots_expected) packaged=$($visualContract.screenshots_packaged) missing=$($visualContract.screenshots_missing)"
    }
    else {
        Add-ExecutionLog "VISUAL_ARTIFACT_CONTRACT_PASS expected=$($visualContract.screenshots_expected) packaged=$($visualContract.screenshots_packaged)"
    }
    ...
    try {
        ...
        $runReportStatus = if ([string]::IsNullOrWhiteSpace([string]$Assembled.overall_status)) { 'FAIL' } else { [string]$Assembled.overall_status }
        $runReportErrorClass = if ($runReportStatus -eq 'FAIL') { 'BUNDLE_EXECUTION_FAILURE' } else { 'NONE' }
        $runReportFailedStep = if ($runReportStatus -eq 'FAIL') { 'BUNDLE_PIPELINE' } else { 'NONE' }
        if ([string]$visualContract.status -eq 'FAIL') {
            $runReportFailedStep = 'VISUAL_ARTIFACT_CONTRACT'
        ...
        Write-RunReportJson -FinalStatus 'FAIL' -RunId 'bundle-run' -FailedStep 'WRITING_STAGE' -ErrorClass 'WRITING_EXCEPTION' -Message "RUN_REPORT generated from writing-stage exception: $($_.Exception.Message)" -VisualArtifacts $visualContract
    }

    Copy-DirectoryContents -Source $bundleRoot -Destination $legacyBundleRoot | Out-Null
    Copy-DirectoryContents -Source $packagedScreenshotsRoot -Destination (Join-Path $legacyBundleRoot 'screenshots') | Out-Null
